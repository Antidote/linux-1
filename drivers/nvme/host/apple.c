...
#define APPLE_NVME_Q_SQES 6

#define APPLE_ANS_BOOT_TIMEOUT msecs_to_jiffies(MSEC_PER_SEC)
 ...
 * looks rather silly at first. This hardware is howver designed for a kernel
...
static void *apple_nvme_rtkit_dma_alloc(void *cookie, size_t size,
...
static void apple_nvme_rtkit_dma_free(void *cookie, size_t size, void *cpu_addr,
				      dma_addr_t dma_handle)
{
	...
	ret = apple_sart_remove_allowed_region(anv->sart, dma_handle, size);
	if (ret)
		dev_warn(anv->dev,
			 "apple_sart_remove_allowed_region failed with %d.",
			 ret);
}
...
static const struct apple_rtkit_ops apple_nvme_rtkit_ops = {
	...
	.shmem_alloc = apple_nvme_rtkit_dma_alloc,
	.shmem_free = apple_nvme_rtkit_dma_free,
};
				  ...
				  struct nvme_command *cmd)
{
	...
	memcpy(q->sq_cmds + (tag << APPLE_NVME_Q_SQES), cmd, sizeof(*cmd));
	writel(tag, q->sq_db);
}
...
static bool apple_nvme_check_cq(struct apple_nvme_queue *q, bool force)
{
	bool found = false;
...
static irqreturn_t apple_nvme_irq(int irq, void *data)
{
	...
	if (apple_nvme_check_cq(&anv->ioq, false))
		handled = true;
	if (apple_nvme_check_cq(&anv->adminq, false))
		handled = true;
...
static void apple_nvme_disable(struct apple_nvme *anv, bool shutdown)
{
	...
	if (!dead) {
		...
		if (READ_ONCE(anv->adminq.enabled)) {
			if (shutdown)
	...
	spin_lock_irqsave(&anv->lock, flags);
	apple_nvme_check_cq(&anv->ioq, true);
	apple_nvme_check_cq(&anv->adminq, true);
	spin_unlock_irqrestore(&anv->lock, flags);
...
static void apple_nvme_abort_endio(struct request *req, blk_status_t error)
{
	struct apple_nvme_iod *iod = blk_mq_rq_to_pdu(req);
	struct apple_nvme_queue *q = iod->q;
	struct apple_nvme *anv = queue_to_apple_nvme(q);

	dev_warn(anv->dev, "Abort status: 0x%x", nvme_req(req)->status);
	atomic_inc(&anv->ctrl.abort_limit);
						   ...
						   bool reserved)
{
	...
	/* just restart the timer if a reset is already in progress */
	if (anv->ctrl.state == NVME_CTRL_RESETTING)
		return BLK_EH_RESET_TIMER;

	/* shutdown immediately on timeouts while starting */
	if (anv->ctrl.state == NVME_CTRL_CONNECTING)
		nvme_change_ctrl_state(&anv->ctrl, NVME_CTRL_DELETING);

	if (anv->ctrl.state == NVME_CTRL_DELETING) {
		dev_warn_ratelimited(anv->ctrl.device,
				     "I/O %d timeout, disable controller\n",
				     req->tag);
	...
	/* check if we just missed an interrupt */
	spin_lock_irqsave(&anv->lock, flags);
	apple_nvme_check_cq(q, false);
	spin_unlock_irqrestore(&anv->lock, flags);
	if (blk_mq_request_completed(req)) {
		dev_warn(anv->dev, "I/O %d timeout, completion polled\n",
			 req->tag);
		return BLK_EH_DONE;
	}
	 ...
	 */
	if (q->is_adminq || iod->aborted) {
		dev_warn(anv->dev, "I/O %d timeout, reset controller\n",
			 req->tag);
		nvme_req(req)->flags |= NVME_REQ_CANCELLED;
		apple_nvme_disable(anv, false);
		nvme_reset_ctrl(&anv->ctrl);
	...
	if (atomic_dec_return(&anv->ctrl.abort_limit) < 0) {
		atomic_inc(&anv->ctrl.abort_limit);
		return BLK_EH_RESET_TIMER;
	}
	iod->aborted = 1;
	...
	dev_warn(anv->dev, "I/O %d timeout, aborting\n", req->tag);

	abort_req =
		nvme_alloc_request(anv->ctrl.admin_q, &cmd, BLK_MQ_REQ_NOWAIT);
	if (IS_ERR(abort_req)) {
		atomic_inc(&anv->ctrl.abort_limit);
		return BLK_EH_RESET_TIMER;
	}
	...
	blk_execute_rq_nowait(NULL, abort_req, 0, apple_nvme_abort_endio);
			   ...
			   struct io_comp_batch *iob)
{
	...
	spin_lock_irqsave(&anv->lock, flags);
	found = apple_nvme_check_cq(q, false);
	spin_unlock_irqrestore(&anv->lock, flags);
...
static void apple_nvme_reset_work(struct work_struct *work)
{
	...
	ret = nvme_disable_ctrl(&anv->ctrl);
	if (ret)
		goto out;
	...
	dev_dbg(anv->dev, "ANS booteed succesfully.");
	...
	lo_hi_writeq_relaxed(anv->adminq.tcb_dma_addr,
			     anv->mmio_nvme + APPLE_NVMMU_BASE_ASQ);
	lo_hi_writeq_relaxed(anv->ioq.tcb_dma_addr,
			     anv->mmio_nvme + APPLE_NVMMU_BASE_IOSQ);
	...
	dev_dbg(anv->dev, "Switching state to CONNECTING");
	...
	dev_dbg(anv->dev, "nvme_init_ctrl_finish");
	...
	dev_dbg(anv->dev, "Controller state switched to live.");
				  ...
				  struct apple_nvme_queue *q)
{
	...
	q->sq_cmds = dmam_alloc_coherent(anv->dev, depth << APPLE_NVME_Q_SQES,
					 &q->sq_dma_addr, GFP_KERNEL);
	if (!q->sq_cmds)
		return -ENOMEM;

	/*
	 * we need the full queue depth here because the NVMMU only has a single
	 * shared queue depth configuration for both queues
	 */
...
static int apple_nvme_probe(struct platform_device *pdev)
{
	...
	anv->prp_page_pool =
		dmam_pool_create("prp list page", anv->dev, NVME_CTRL_PAGE_SIZE,
