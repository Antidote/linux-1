...
enum { APPLE_RTKIT_STATE_BOOT_WAIT,
       APPLE_RTKIT_STATE_BOOT_FAILED,
       APPLE_RTKIT_STATE_RUNNING,
       APPLE_RTKIT_STATE_CRASHED,
       APPLE_RTKIT_STATE_SHUTDOWN,
};
				    ...
				    struct apple_rtkit_shmem *bfr)
{
	...
	if (rtk->ops->flags & APPLE_RTKIT_SHMEM_OWNER_LINUX) {
		if (rtk->ops->shmem_free)
			rtk->ops->shmem_free(rtk->cookie, bfr->size,
					     bfr->buffer, bfr->iova);
		else
			dma_free_coherent(rtk->dev, bfr->size, bfr->buffer,
					  bfr->iova);
	} else if (rtk->ops->flags & APPLE_RTKIT_SHMEM_OWNER_RTKIT) {
...
static void apple_rtkit_crashlog_rx(struct apple_rtkit *rtk, u64 msg)
{
	 ...
	 * the buffer is normal memory and no iomem for e.g. the SMC
	 */
